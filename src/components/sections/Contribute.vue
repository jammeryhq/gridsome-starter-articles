<template>
  <section class="bg-shark-500 body-font text-center sm:text-left">
    <div class="container py-12 mx-auto">
      <div class="w-full lg:w-3/4 flex flex-col md:flex-row md:items-center items-start mx-auto">
        <p
          class="flex-grow sm:pr-16 text-2xl font-medium title-font text-gray-200">
          Did you know that you can easily create your own articles? Checkout our contribution guide.
        </p>
        <g-link
          to="/pages/contribute"
          class=" self-center md:self-auto flex-shrink-0 text-white bg-smalt-blue-500 border-0 py-2 px-8 focus:outline-none hover:bg-smalt-blue-300 hover:text-black rounded text-lg mt-10 md:mt-0">
          Contribute now
        </g-link>
      </div>
    </div>
  </section>
</template>

<script>
export default {};
</script>

<style>
</style>
